...
set("n", "<leader>cd", "<cmd>cd %:p:h<cr>", desc("change current directory to the file in the buffer"))

set("n", "<leader>rs", removeTrailingWhiteSpace, desc("Remove all trailing whitespace"))
...
set("n", "<leader>o", "%O", desc("Insert after the last line in the enclosing brackets"))
...
set("n", "<leader>fG", function()
	liveGrep(false)
end, desc("Live grep but also look for hidden files"))
...
set("n", "<leader>bd", [[<cmd>%bd<bar>e#<bar>bd#<cr><bar>'"]], desc("Close all buffers except current one"))

set("n", "Y", "yg$", desc("Yank analogue of D"))
set("n", "<leader>x", "<cmd>silent !chmod +x %<cr>", desc("Make current file executable"))
set("n", "<leader>X", "<cmd>!./%<cr>", desc("Execute current file"))

set({ "n", "v" }, "<leader>y", '"+y', desc("Yank to clipboard register"))
set("n", "<leader>Y", '"+Y')

set("n", "<leader>cc", 'gg"+yG', desc("Yank the entire buffer into the clipboard"))
set("n", "<leader>p", '"+p', desc("Paste clipboard contents"))
set("n", "<leader>cv", 'ggVG"+p', desc("Replace the buffer with clipboard contents"))
